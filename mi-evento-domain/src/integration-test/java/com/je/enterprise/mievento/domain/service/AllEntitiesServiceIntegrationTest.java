...
public class AllEntitiesServiceIntegrationTest {
	...
	private UserEntity createUserWithEvents() {
		...
		LocationEntity locationHusband = new LocationEntity(CountryCode.AR,"Buenos Aires","CABA",new StreetAddressEntity("callea", BigDecimal.ONE, null, "Palermo"),StringUtils.EMPTY,StringUtils.EMPTY);
		
		LocationEntity locationWedding = new LocationEntity(CountryCode.AR,"Buenos Aires","CABA",new StreetAddressEntity("calleZ", BigDecimal.ONE, null, "Recoleta"),StringUtils.EMPTY,StringUtils.EMPTY);
		wedding.setPlace(new PlaceEntity("id1", "Salon Pueyrredon", "El mejor salon de todos loco.Rompermos todo", locationWedding, "salonP@gmail.com", "156545787", "01144578954", BigDecimal.ONE, BigDecimal.TEN, BigDecimal.valueOf(250), BigDecimal.valueOf(19), BigDecimal.valueOf(75),Arrays.asList("http://picture2"),ProviderType.ALQUILER,null,null,Boolean.FALSE));
		...
		wedding.setPresents(Lists.<PresentsEntity>newArrayList(new PresentsEntity(Lists.newArrayList("Vajilla"), BigDecimal.ONE,new CommercialLocationEntity("Falabella",CountryCode.AR,"Buenos Aires","CABA",new StreetAddressEntity("callePresent", BigDecimal.ONE, null, "Recoleta")),null)));
		wedding.setProviders(Lists.<ProviderEntity>newArrayList());
		
		LocationEntity locationFotografo = new LocationEntity(CountryCode.AR,"Buenos Aires","CABA",new StreetAddressEntity("calleZ", BigDecimal.ONE, null, "Recoleta"),StringUtils.EMPTY,StringUtils.EMPTY);
		wedding.getProviders().add(new ProviderEntity("id2", "Fotografo Carlitox", "Carlitox HOOO", locationFotografo, "Fc@gmail.com", "15548798","454879865", BigDecimal.ONE, BigDecimal.TEN,Arrays.asList("http://image1"),ProviderType.ALQUILER,null));
		
		user.getEvents().add(wedding);
		
		
		CommercialLocationEntity locationEntregaTPFinal = new CommercialLocationEntity("Facultad Ingenieria",CountryCode.AR,"Buenos Aires","CABA",new StreetAddressEntity("Paseo Coloon", BigDecimal.valueOf(750), "esquina Independencia", "San Telmo"));
